...
export default function SubNames({ parentDomain, owner }: Props) {
  ...
  const { isError, error: execError, isLoading: isConfirming, isSuccess: isConfirmed } =
  ...
  useEffect(() => {
    if (parentDomain){
      getDomainAddress(parentDomain).then(() => fetchData()).catch(console.error);
    }
  }, []);
  ...
  useEffect(() => {
    if (walletError) {
      ...
      toast.error("Create SubName failed", {
        description: (walletError as BaseError).shortMessage || walletError.message,
      });
    }
    if (isError) {
      toast.dismiss();
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <DialogFooter className="mt-4">
                <Button
                  ...
                  onClick={handleSave} disabled={isConfirming}
                  type={"button"}
                >
                  {isConfirming ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : "Save changes"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
